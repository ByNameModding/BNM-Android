...
void *set_fov(float value) {
    ...
    if (Camera$$get_main && Camera$$get_fieldofview && Camera$$set_fieldofview) {
        ...
        if (BNM_IsUnityObjectAlive(mainCamera)) {
...
Field<monoString *> PlayerName; // private String PlayerName;
Field<monoList<monoString *> *> PlayersName; // private static List<String> PlayersName;
void (*old_Update)(...);
void Update(void *instance){
    ...
    if (BNM_IsSameUnityObject(myPlayer, instance)){ // Check is unity object (UnityEngine.Object) are same
        // Do sth
    }

    if (BNM_IsUnityObjectAlive(myPlayer)) { // Check is unity object (UnityEngine.Object) not null
        ...
        if (!setName){
            PlayerName.setInstance(myPlayer);
            LOGIBNM(OBFUSCATE_BNM("myPlayer old name is %s"), PlayerName()->c_str());
            PlayerName = CreateMonoString(OBFUSCATE_BNM("ByNameModding_Player"));
...
namespace geokar2006 {
    ...
    class BNM_DllExampleClass {
        //NewClassInit(dll, namespace, class, parent class namespace (maybe ""), parent class name (maybe ""), parent class size, class type(set to 0)); // for System.Object only sizeof(Il2CppObject)
        NewClassWithDllInit("Assembly-CSharp", "geokar2006", BNM_DllExampleClass, "", "", sizeof(Il2CppObject), 0);
        void Start(){
...
void hack_thread() {
    ...
    PlayerName = FPSControler.GetFieldByName<monoString *>(OBFUSCATE_BNM("PlayerName"));
    LocalPlayer = FPSControler.GetFieldByName<void *>(OBFUSCATE_BNM("LocalPlayer"));
    PlayersName = FPSControler.GetFieldByName<monoList<monoString *> *>(OBFUSCATE_BNM("PlayersName"));
