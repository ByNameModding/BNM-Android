...
#pragma pack(push, 1)
    ...
    struct Class {
        ...
        template<typename T>
        BNM::Structures::Mono::monoArray<T> *NewArray(IL2CPP::il2cpp_array_size_t length = 0) const {
            if (!_data) return nullptr;
            TryInit();
            return (BNM::Structures::Mono::monoArray<T> *) ArrayNew(_data, length);
        }
        ...
        Structures::Mono::monoList<T> *NewList() const {
            ...
            auto *lst = (BNM::Structures::Mono::monoList<T> *) NewListInstance();
            if (!lst) {
        ...
        void *CreateNewObjectParameters(Parameters ...parameters) const;

        template<typename ...Parameters>
        void *CreateNewObjectTypes(const std::initializer_list<std::string_view> &parameterNames, Parameters ...parameters) const;
        ...
        [[nodiscard]] inline std::string str() const {
            ...
            TryInit();
            std::string data{};
            if (_data->declaringType) data += Class(_data->declaringType).str() + OBFUSCATE_BNM("<-");
            data += OBFUSCATE_BNM("[");
...
#pragma pack(pop)
    ...
    struct CompileTimeClass {
        ...
        struct _GenericInfo : _BaseInfo {
            ...
            std::initializer_list<CompileTimeClass> _types{};
        };
...
#pragma pack(pop)
    ...
    constexpr ConstexprCompileTimeClass GetType(CompileTimeClass::ModifierType modifier = CompileTimeClass::ModifierType::None) noexcept {
        ...
        else if constexpr (std::is_same<T, intptr_t>::value)
        ...
        else if constexpr (std::is_same<T, BNM::IL2CPP::Il2CppString *>::value || std::is_same<T, monoString *>::value)
        ...
        else if constexpr (std::is_same<T, BNM::UnityEngine::Object *>::value)
