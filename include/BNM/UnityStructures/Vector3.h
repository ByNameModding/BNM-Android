...
namespace BNM::Structures::Unity {
    ...
    struct Vector3 {
        ...
        inline static Vector3 FromString(const std::string&);

        inline Vector3 get_orthogonal() { return Orthogonal(*this); }
        inline float get_magnitude() { return Magnitude(*this); }
        inline Vector3 get_normalized() { return Normalize(*this); }
        inline float get_sqrMagnitude() { return SqrMagnitude(*this); }
    ...
    inline Vector3 operator-(const Vector3& lhs, const Vector3& rhs) { return {lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z}; }
    inline Vector3 operator*(const Vector3& inV, const float s) { return {inV.x * s, inV.y * s, inV.z * s}; }
    inline Vector3 operator*(const float s, const Vector3& inV) { return {inV.x * s, inV.y * s, inV.z * s}; }
    inline Vector3 operator*(const Vector3& lhs, const Vector3& rhs) { return {lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z}; }
    ...
    Vector3 Vector3::FromString(const std::string& str) {
        ...
        return {std::stof(commands[0]), std::stof(commands[1]), std::stof(commands[2])};
    }
}
