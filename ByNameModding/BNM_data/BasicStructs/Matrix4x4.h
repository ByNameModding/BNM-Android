...
namespace BNM::Structures::Unity {
    ...
    struct Matrix4x4 {
        ...
        Matrix4x4() {}
        inline explicit Matrix4x4(InitIdentity) { SetIdentity(); }
        ...
        inline explicit Matrix4x4(const float data[16]) {
            for (int i = 0; i < 16; i++)
            m_Data[i] = data[i];
        }
        ...
        inline bool IsIdentity(float threshold = Vector3::kEpsilon) const {
        ...
        inline Vector3 GetAxisX() const { return {Get(0, 0), Get(1, 0), Get(2, 0)}; }
        inline Vector3 GetAxisY() const { return {Get(0, 1), Get(1, 1), Get(2, 1)}; }
        inline Vector3 GetAxisZ() const { return {Get(0, 2), Get(1, 2), Get(2, 2)}; }
        inline Vector3 GetAxis(int axis) const { return {Get(0, axis), Get(1, axis), Get(2, axis)}; }
        inline Vector3 GetPosition() const { return {Get(0, 3), Get(1, 3), Get(2, 3)}; }
        ...
        inline Vector4 GetRow(int row) const { return {Get(row, 0), Get(row, 1), Get(row, 2), Get(row, 3)}; }
        inline Vector4 GetColumn(int col) const { return {Get(0, col), Get(1, col), Get(2, col), Get(3, col)}; }
